...
namespace AWSIM.AWAnalysis.TraceExporter.Objects
{
    public class PerceptionObject
    {
        ...
        public string DumpMaudeStr()
        {
            ...
            string predictPathStr = "";
            if (ConfigLoader.CapturePredictionPaths())
            {
                if (predict_paths == null || predict_paths.Length == 0)
                    predictPathStr = "pdpath: nil";
                else
                {
                    predictPathStr = $"pdpath: {{{predict_paths[0].DumpMaudeStr()}}}";
                    for (int i = 1; i < predict_paths.Length; i++)
                    {
                        predictPathStr += $" {{{predict_paths[i].DumpMaudeStr()}}}";
                    }
                }
            }
