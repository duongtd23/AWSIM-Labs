    ...
    public class CustomNPCSpawningManager
    {
        ...
        public void UpdateNPCs()
        {
            if (Manager() != null)
            {
                ...
                if (egoStartMovingTime <= 0 &&
                    CustomSimUtils.MagnitudeIgnoreYAxis(EgoSingletonInstance.AutowareEgoCarAdapter.Velocity) > 0.15f)
                ...
                else if (idelay is NPCDelayDistance delayDistance)
                {
                    if (EgoSingletonInstance.AutowareEgoCarAdapter.Velocity.magnitude > 0.1f &&
                        CustomSimUtils.LongitudeDistance(
                            ...
                            EgoSingletonInstance.AutowareEgoCarAdapter.Rotation * Vector3.forward,
        ...
        private void UpdateCutoutLeadingNPC()
        {
            ...
            if (leadingNPC != null)
            {
                ...
                {
                    float currentSpeed = EgoSingletonInstance.AutowareEgoCarAdapter.Velocity.magnitude;
                    ...
                    float actual_dx0 = CustomSimUtils.DistanceIgnoreYAxis(EgoSingletonInstance.AutowareEgoCarAdapter.Position, cutoutNPC.Position);
        ...
        private void UpdateDecelerationNPC()
        {
            ...
            if (internalState?.SpeedMode == NPCVehicleSpeedMode.STOP)
            {
                ...
                if (unspawnNPC != null)
                {
                    float desired_dx0 = Mathf.Min(EgoSingletonInstance.AutowareEgoCarAdapter.Velocity.magnitude, decelNPC.Velocity.magnitude) *
                                        ConfigLoader.Config().TimeHeadWay;
                    float actual_dx0 = CustomSimUtils.DistanceIgnoreYAxis(EgoSingletonInstance.AutowareEgoCarAdapter.Position, decelNPC.Position);
                    actual_dx0 -= (float)EgoSingletonInstance.GetEgoDetailInfo().extents.z +
                                  (float)EgoSingletonInstance.GetEgoDetailInfo().center.z;
