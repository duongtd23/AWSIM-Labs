...
namespace AWSIM_Script.Parser
{
    public static class ParserUtils
    {
        ...
        {
            if (!ConfigLoader.Config().AWSIMConfig.AllMapNames.Contains(mapName))
            {
                Debug.LogError($"Map name {mapName} not found. " +
                               $"Please specify one of the maps {ConfigLoader.Config().AWSIMConfig.AllMapNames}. " +
                               $"Using the first one as default.");
                return ConfigLoader.Config().AWSIMConfig.AllMapNames[0];
            }
            return mapName;
        }
        
        public static string ValidateEgoModelName(string modelName)
        {
            if (!ConfigLoader.Config().AWSIMConfig.AllEgoModelNames.Contains(modelName))
            {
                Debug.LogError($"Ego model name {modelName} not found. " +
                               $"Please specify one of the models {ConfigLoader.Config().AWSIMConfig.AllEgoModelNames}. " +
                               $"Using the first one as default.");
                return ConfigLoader.Config().AWSIMConfig.AllEgoModelNames[0];
            }
            return modelName;
        }
