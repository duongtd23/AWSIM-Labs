...
namespace AWSIM_Script.Parser
{
	public class ScenarioParser
	{
        ...
        public const string TURN_AROUND = "turn-around";
        ...
        public Simulation Parse()
        {
            ...
            if (runFunc.Parameters.Count < 3)
                throw new InvalidScriptException("Invalid arguments passed for function run. " +
                                                 "Map name, Ego, NPC lists are required.");
            
            // 1st arg: map name
            bool ok = ParseString(runFunc.Parameters[0].children[0], out string mapName);
            if (!ok)
                throw new InvalidScriptException("Cannot parse map name from: " + runFunc.Parameters[0].children[0].GetText());
            simulation.MapName = ParserUtils.ValidateMapName(mapName);

            // 2nd arg: Ego
            // if the first param is Ego
            if (runFunc.Parameters[1].children?[0] is VariableExpContext varExp)
            {
                // if it is an underscore _ -> ignore Ego
                if (varExp.GetText() != IGNORE_EXP)
                {
            ...
            else if (runFunc.Parameters[1].children?[0] is FunctionExpContext funcExp)
            {
                egoFuncExp = funcExp;
            }
            else
                throw new InvalidScriptException("Invalid arguments passed for function run: " + runFunc.Parameters[1].GetText());

            // 3rd arg: NPC list (use [] to skip)
            if (runFunc.Parameters[2].children?[0] is ArrayExpContext arrayExpContext)
            {
                npcsExpContexts = ParserUtils.ParseArray(arrayExpContext);
            }
            else
                throw new InvalidScriptException("Expected an array of NPCs (possibly empty), but get: " + runFunc.Parameters[2].GetText());
        ...
        private bool RetrieveEgo(FunctionExpContext egoFuncExp, ref Simulation simulation)
        {
            ...
            if (func.Parameters.Count < 3)
                throw new InvalidScriptException("Ego function must have at least 3 arguments (model name, initial position, and goal): " +
                    ParserUtils.ToString(func.Parameters));

            // 1st arg: always model name
            bool ok = ParseString(func.Parameters[0].children[0], out string modelName);
            if (!ok)
                throw new InvalidScriptException("Cannot parse model name from: " + func.Parameters[0].children[0].GetText());
            modelName = ParserUtils.ValidateEgoModelName(modelName);
            
            // 2nd arg: always initial position
            IPosition initPosition = ParsePosition(func.Parameters[1].children[0]);

            // 3rd arg: always goal
            IPosition goal = ParsePosition(func.Parameters[2].children[0]);
            var egoSettings = new EgoSettings(initPosition, goal);
            egoSettings.ModelName = modelName;

            // 4th arg (optional): setting options, e.g., max velocity
            if (func.Parameters.Count > 3)
            {
                ParseEgoConfig(func.Parameters[3].children[0], ref egoSettings);
            }
            ...
            if (node is ConfigExpContext configExp)
            {
                switch (configExp.children[0].GetText())
                {
                    ...
                    case DELAY_MOVE_UNTIL_EGO_MOVE:
                        ...
                        return 0;
                    case TURN_AROUND:
                        Debug.LogError("Turn around only support pedestrian!");
                        break;
                    default:
